    ...
    <div style={{ margin: "8px 20px" }}>
      <div className={styles["frame-container"]}>
        ...
        <iframe
          ...
          src={state.cid.includes("https") ? state.cid : get(state.cid)}
          ref={ref}
          style={{ opacity: 0 }}
        />
      </div>
      <div className={styles["game-details"]}>
        <div className={styles["game-header"]}>
          <div className={styles["game-tile"]}>
            ...
            <div className={styles["game-tags"]}>
              <a className={styles["game-tag"]}>
                {state.tags.length > 1
                  ? Tags.find((e) => e.id.toString() === state.tags[0])?.name ??
                    "Web3"
                  : ""}
              </a>
        ...
        <h3>Release Date</h3>
        <p>
          {convertMonth(state.date.getMonth()) + " " + state.date.getFullYear()}
        </p>
        <h3>Developer</h3>
        <p>
          {state.title} is developed by {state.creator}
        </p>
        <h3>Platform</h3>
        <p>Web browser</p>
          ...
          {state.tags.map((tag, i) => {
            return (
              <div key={i} className={styles["tag-tile"]}>
                ...
                />
                {Tags.find((e) => e.id.toString() === tag)?.name ?? "Web3"}
              </div>
            );
          })}
        </div>
